...
# Make sure we hava qapm_table with these columns, otherwise make it using this query in mysql
# qurey = '''
...
def clip_hdf_using_indices(hdf_path, indices):
    ...
    if not df.empty:
        ...
        df.to_sql('qapm_table', con=engine, if_exists='append', index=False)
...
hdf_sample_path = os.path.normpath(os.path.join('.', '..', '..', 'data', 'sample', 'AMSR_37V_AM_FT_2021_day003_v05.1.h5'))
...
print("HDF Directory:", hdf_directory)


hdf_files = [os.path.join(hdf_directory, f) for f in os.listdir(hdf_directory) if fnmatch.fnmatch(f, '*QA*PM*.h5')] 
print(hdf_files)
