...
def clip_hdf_using_indices(hdf_path, indices):
    ...
    if not df.empty:
        ...
        df.to_sql('acpm_table', con=engine, if_exists='append', index=False)
...
hdf_files = [os.path.join(hdf_directory, f) for f in os.listdir(hdf_directory) if fnmatch.fnmatch(f, '*_PM_accuracy_*.h5')] 
